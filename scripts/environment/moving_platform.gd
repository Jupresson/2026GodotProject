...
func _func_godot_apply_properties(entity_properties: Dictionary) -> void:
	move_distance = entity_properties.get("move_distance", move_distance) as float
	move_time = entity_properties.get("move_time", move_time) as float
	move_direction = entity_properties.get("move_direction", move_direction) as Vector3
...
func _start_movement() -> void:
	...
	platform_tween.set_loops() # infinite ping-pong
