extends Character # extends from chrachter base class that has eg: healt system..
...
# When true, controls are locked while character is dead.
@export var disable_input_when_dead: bool = true
# Instantly kills the player when dropping below this world Y value.
@export var lethal_fall_y: float = -100.0
...
func _ready() -> void:
	...
	# Initialize Character base state (health, enabled-state, spawn hook).
	super._ready()

	_update_camera_visual(1.0)

func _on_spawn() -> void:
	# Sync runtime movement/camera state whenever the character spawns or revives.
	_jump = false
	_mouse_accum = Vector2.ZERO
	velocity = Vector3.ZERO
	_yaw_deg = rad_to_deg(rotation.y)
	_pitch_deg = clamp(_pitch_deg, max_look_down, max_look_up)
	_prev_pos = global_position
	_curr_pos = global_position
	_prev_yaw = _yaw_deg
	_curr_yaw = _yaw_deg
	...
	if is_enabled and _has_focus:
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)

func _on_death() -> void:
	# Character base calls this when health reaches zero.
	_jump = false
	_mouse_accum = Vector2.ZERO
	velocity = Vector3.ZERO
	Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
...
func _unhandled_input(event: InputEvent) -> void:
	if not is_enabled:
		return
	if disable_input_when_dead and not is_alive:
		return
...
func _physics_process(delta: float) -> void:
	...
	if is_alive and global_position.y <= lethal_fall_y:
		kill()
		return
