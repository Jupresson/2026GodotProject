...
func _on_scale_slider_value_changed(value):
	...
	if get_viewport().get_scaling_3d_mode() == Viewport.SCALING_3D_MODE_FSR2 and safe_value > 100.0:
		safe_value = 100.0
		scale_slider.set_value_no_signal(safe_value)

	var Resolution_Scale = safe_value/100.00
	var Resolution_Text = str(round(get_window().get_size().x*Resolution_Scale))+"x"+str(round(get_window().get_size().y*Resolution_Scale))
	
	scale_label.set_text(str(safe_value)+"% - "+ Resolution_Text)
...
func _on_scaler_item_selected(index):
	...
	match index:
		1:
			...
			scale_slider.max_value = 200.0
			scale_slider.set_editable(true)
			fsr_options.hide()
		2:
			if scale_slider.value > 100.0:
				scale_slider.set_value_no_signal(77.0)
				_on_scale_slider_value_changed(77.0)
			scale_slider.max_value = 100.0
